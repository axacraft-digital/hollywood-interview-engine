#!/usr/bin/env node
/**
 * Exports the canonical instrument JSON to exports/ along with a HOW-TO-EDIT
 * sidecar so the hiring authority can restructure questions without needing
 * to read the React code.
 *
 * Usage:
 *   node scripts/export-instrument.mjs                       # default: vintage-mechanic-v1
 *   node scripts/export-instrument.mjs <instrument-id>       # explicit
 */
...
const id = process.argv[2] || 'vintage-mechanic-v1';
const sourcePath = path.join(root, 'instruments', `${id}.json`);
const outDir = path.join(root, 'exports');
const outJson = path.join(outDir, `${id}.json`);
const outGuide = path.join(outDir, 'HOW-TO-EDIT.md');
...
const instrument = JSON.parse(fs.readFileSync(sourcePath, 'utf8'));

// Stamp metadata about when this export was produced so revisions are traceable.
const stamped = {
  _exportedAt: new Date().toISOString(),
  _exportedFrom: `instruments/${id}.json`,
  _note: 'See HOW-TO-EDIT.md in this folder for what each field does.',
  ...instrument,
};

fs.writeFileSync(outJson, JSON.stringify(stamped, null, 2) + '\n');

const guide = `# How to edit \`${id}.json\`
...
fs.writeFileSync(outGuide, guide);

console.log(`✓ exported instrument ${id}`);
console.log(`  → ${path.relative(root, outJson)}`);
console.log(`  → ${path.relative(root, outGuide)}`);
console.log(`  questions: ${instrument.groups.reduce((n, g) => n + g.questions.length, 0)}`);
console.log(`  version:   ${instrument.version}`);
